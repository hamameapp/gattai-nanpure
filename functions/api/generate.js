...
// 速い合体ナンプレ生成API + 解答返却（Cloudflare Pages Functions）
//
// - 1つの“無限”完成パターン valueAt(R,C) を作り、各盤は (ox,oy) からの 9x9 ウィンドウとして切り出す
//   → 共有セルは必ず同じ数字（合体ルール厳守）
// - 各盤ごとにヒントを削る（carve）。最後に共有セルの与え/非与えを整合
// - バックトラック不要で O(盤数×81)。CPU制限（Error 1102）を回避
// - 応答で puzzle.boards[].solution に完成盤（解答）も返す
//
// 受信JSON: { layout:[{id,x,y}...], adShown?:boolean, difficulty?:"easy"|"normal"|"hard" }
// 応答JSON: { ok:true, puzzle:{ boards:[{id,x,y,grid:number[9][9],solution:number[9][9]}...] } }

const GRID = 9;
// ★フロントの CELL と同じ（script.js が 30 なので 30）
const CELL_PX = 30;
