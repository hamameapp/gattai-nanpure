// script.js — Cloudflare Pages + Functions 用 複数盤対応フロント
// 9x9 複数ボードをキャンバス上に配置して /api/generate に layout をまとめて渡す
...
(() => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const exportTextButton = byId('exportTextButton'); // 無いなら無視

    // ====== 定数 ======
    const GRID = 9;
    const CELL = 40; // 1マスのピクセル
    const BOARD_PIX = GRID * CELL; // 360px
    const SNAP = CELL; // 盤のドラッグはセル単位でスナップ
    ...
    let editMode = true; // レイアウト編集モード
    let activeSquareId = null;
    let activeCell = null; // {id,r,c} … プレイ時に選択中セル
    ...
    function newSquare(x, y) {
      ...
      return {
        ...
        w: BOARD_PIX,
        h: BOARD_PIX,
        problemData: createEmptyGrid(), // 0=空欄、1..9=問題の数字
        userData: createEmptyGrid(),    // ユーザー入力
        checkData: createEmptyGrid(),   // チェック結果（0=OK, 1=NG）
        solutionData: createEmptyGrid() // 将来用
      };
    }

    function nextId() {
      let m = 0;
      for (const s of squares) m = Math.max(m, Number(s.id) || 0);
      return String(m + 1);
    }
    ...
    function updateButtonStates() {
      ...
      if (solveButton) {
        solveButton.disabled = true; // まだ未実装
        solveButton.title = 'サーバが解答を返すようになったら有効化します';
      }
    ...
    function drawBoard(s) {
      ...
      for (let i = 1; i < GRID; i++) {
        ...
        // 縦
        ctx.beginPath();
        ctx.moveTo(gx + 0.5, s.y);
        ctx.lineTo(gx + 0.5, s.y + s.h);
        ctx.stroke();
        // 横
        ctx.beginPath();
        ctx.moveTo(s.x, gy + 0.5);
        ctx.lineTo(s.x + s.w, gy + 0.5);
        ctx.stroke();
      }
      // 太線（3x3）
      ctx.lineWidth = 2;
      ...
      for (let i = 0; i <= GRID; i += 3) {
        const gx = s.x + i * CELL + 0.5;
        const gy = s.y + i * CELL + 0.5;
        // 縦
        ctx.beginPath();
        ctx.moveTo(gx, s.y);
    ...
    checkButton && checkButton.addEventListener('click', () => {
      ...
      setStatus('重複チェックを実行しました（赤＝矛盾）');
      draw();
    });

    solveButton && solveButton.addEventListener('click', () => {
      alert('解答表示はまだ未実装です（サーバが solution を返すようになったら有効化します）。');
    });
    ...
    exportTextButton && exportTextButton.addEventListener('click', () => {
      ...
      a.href = url; a.download = 'gattai_export.json';
      a.click();
    ...
    async function handleGenerateProblem() {
      if (squares.length === 0) { alert('まず「盤を追加」してください'); return; }
      ...
      try {
        ...
        const boards = await generateFromServer(layout, false, 'normal'); // adShown=false
        renderBoards(boards);
